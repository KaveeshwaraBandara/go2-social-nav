...
DISPATCHING INTENT   (DOWNSTREAM CHANGE - go2-social-nav)
---------------------------------------------------------
This module never assumes what a command *does*. run() takes a `dispatch`
callback with the signature:

    dispatch(intent: GestureIntent) -> None

Upstream this was `dispatch(command: str, confidence: float)`. It now hands over
the frozen contract object from `gesture_intent`, built by intent_adapter, so
that WHO gestured, how sure we are it was them, and where they stand travel with
the label instead of being discarded at the callback boundary.

The default just prints. To drive a robot instead, pass your own. The rule the
signature exists to enforce: this producer emits INTENT, never velocity - the
IT2-FLS owns /cmd_vel, because the whole point is that the same gesture produces
different motion in different contexts. A ROS 2 node becomes roughly:

    from gesture_lab.gesture_control import run

    def dispatch(intent):
        publisher.publish(to_msg(intent))    # confidences NOT thresholded
...
CLASSIFICATION (trained model REQUIRED)
---------------------------------------
Gestures are classified by a RandomForest over windowed motion AND hand-shape
features. The model has an explicit "NONE" class, so it can answer "nothing is
happening" rather than being forced to pick a command for every window of hand
motion.
...
Neither is a contradiction of "confidences are never thresholded": they decide
whether a window is a gesture AT ALL, upstream of the label. Once a label
exists, its probability rides through to the controller untouched.
...
DOWNSTREAM CHANGE: upstream falls back to a rule-based classifier when no model
is on disk. Here run() raises instead - that fallback emits an older vocabulary
("TURN RIGHT", "MOVE FORWARD") which is not in the contract.
...
# NOTE: upstream also imports classify_gesture_rules here. Dropped: the
# rule-based fallback is unreachable now that run() requires a trained model.
# The function itself is left in gesture_common.py untouched, for upstream
# parity and because train/collect still reference the same module.
from .gesture_model import load_model, predict_gesture
from .hand_landmarker import HandTracker
from .intent_adapter import NO_OPERATOR, make_intent
...
# DOWNSTREAM CHANGE (go2-social-nav): baked into the image at /opt/models rather
# than curl'd into a models/ dir beside the source. See config.py.
POSE_MODEL_PATH = config.POSE_LANDMARKER_PATH


# --------------------------------------------------------------------------
# Default dispatch
# --------------------------------------------------------------------------
# DOWNSTREAM CHANGE (go2-social-nav): dispatch receives a GestureIntent, not
# (command, confidence).
#
# Upstream's two-argument callback is the right shape for a standalone demo but
# it throws away everything the controller needs to act socially: WHO gestured,
# how sure we are it was them, and where they are standing. Those are not
# decorations — operator_confidence is one of the two footprint-of-uncertainty
# inputs the IT2 fuzzifier sizes its intervals from, and a controller that
# cannot locate the operator cannot make COME mean anything different in a
# corridor than in a crowd. Passing the frozen contract object instead keeps
# every consumer honest about what it was given.

def print_dispatch(intent):
    print(
        f"[INTENT] {intent.label:<9} "
        f"gesture={intent.gesture_confidence:.0%} "
        f"operator={intent.operator_confidence:.0%} "
        f"bearing={math.degrees(intent.operator_bearing):+6.1f} deg "
        f"id={intent.operator_id} "
        f"{'ARMED' if intent.armed else 'disarmed'}"
    )
...
def operator_hand(hands_found, operator_wrists):
    ...
    Returns (index, landmarks, distance) or (None, None, None).

    DOWNSTREAM CHANGE (go2-social-nav): the winning DISTANCE is returned as well
    as the hand. Upstream discards it after the threshold test, which reduces a
    graded association ("this hand is 0.03 from the wrist" vs "0.24, right at
    the edge of the radius") to a boolean. That distance is the only evidence
    the pipeline has about how sure the operator lock is, and it becomes
    GestureIntent.operator_confidence — an interval bound the fuzzifier needs.
    Thresholding it away here would leave the controller inventing its own
    uncertainty from a constant, which is exactly how an IT2 system collapses
    into a type-1 one.
    """
    ...
    if best_d is not None and best_d <= config.HAND_WRIST_MAX_DIST:
        return best_i, hands_found[best_i], float(best_d)
    return None, None, None
...
def run(dispatch=print_dispatch, camera_index=0):
    ...
    dispatch(intent: GestureIntent) is called once per recognized gesture.
    Everything else - drawing, arming, operator identity - is internal.

    Raises RuntimeError if no trained model is available (see below).
    """
    # DOWNSTREAM CHANGE (go2-social-nav): refuse to run without a trained model.
    #
    # Upstream degrades gracefully to classify_gesture_rules() so the project
    # runs end-to-end before any data has been recorded - correct for a repo
    # where the output is a print(). It is wrong here, because that fallback
    # emits an OLDER vocabulary ("TURN RIGHT", "MOVE FORWARD") that is not in
    # the contract. Forwarding those would produce confident, well-formed,
    # meaningless intents; make_intent() would reject them one frame later
    # anyway, but deep inside the loop rather than at startup.
    #
    # Fail here, once, with the fix in the message.
    bundle = load_model()
    if bundle is None:
        raise RuntimeError(
            f"No usable gesture model at {config.MODEL_PATH!r}.\n"
            f"\n"
            f"The trained RandomForest is NOT in either repo - upstream's\n"
            f".gitignore excludes *.pkl - so it has to be fetched from the\n"
            f"gesture repo owner and dropped in, or regenerated with\n"
            f"train_gestures.py against Zenodo-27 + IPN-Hand.\n"
            f"\n"
            f"Override the path with $GO2_GESTURE_MODEL.\n"
            f"\n"
            f"(The rule-based fallback is deliberately NOT used here: it emits\n"
            f"an off-contract vocabulary. See CLAUDE.md, 'Phase 11'.)"
        )
    ...
    # DOWNSTREAM ADDITION (go2-social-nav): a monotonic id for the current lock.
    #
    # Same semantics as Track.id, and for the same reason: the controller has to
    # be able to tell "the operator moved" from "a DIFFERENT PERSON now holds
    # the lock". Without it, a release-and-re-enrol is invisible downstream, and
    # a latched mode (FOLLOW, STAY) silently transfers to whoever walked up
    # next. Never reused within a run; -1 means nobody is enrolled.
    operator_id = NO_OPERATOR
    next_operator_id = 0
    ...
    def release(reason):
        nonlocal operator_anchor, operator_lost_since, operator_lms, operator_id
        ...
        operator_lms = None
        # The id dies with the lock. The next enrolment gets a fresh one even if
        # it is the same human walking back into frame - the pipeline cannot
        # know that, and pretending otherwise would let a latched mode survive a
        # gap it should not have survived.
        operator_id = NO_OPERATOR
        if armed:
            disarm(reason)
        else:
    ...
    while True:
        ...
        target_idx, landmarks, wrist_dist = operator_hand(hands_found, operator_wrists)
        ...
        #
        # wrist_dist stays None here, and that is not a gap to be filled: an
        # unenrolled hand has no operator to be associated WITH, so its
        # operator_confidence is genuinely 0.0. Substituting any other number
        # would let a pre-enrolment hand carry authority it does not have.
        if operator_anchor is None and hands_found:
            target_idx, landmarks, wrist_dist = 0, hands_found[0], None
                ...
                if operator_anchor is None:
                    ...
                    if (mode_cooldown <= 0 and people and held_still(
                        ...
                        if best_i is not None:
                            ...
                            buffer.clear()
                            pred_history.clear()
                            print(f"[LOCKED + ARMED] operator {operator_id} enrolled.")
                    ...
                    if (mode_cooldown <= 0 and held_still(
                        ...
                        # DOWNSTREAM CHANGE: no rule-based branch. `bundle` is
                        # guaranteed non-None - run() refuses to start without
                        # it, because the fallback's vocabulary is not in the
                        # contract. classify_gesture_rules stays in
                        # gesture_common for upstream parity; it is unused here.
        ...
        if detected_this_frame is not None:
            ...
            # DOWNSTREAM CHANGE: emit the frozen contract object, not a
            # (label, confidence) pair. `landmarks` and `wrist_dist` are
            # necessarily from THIS frame - detected_this_frame can only be set
            # inside the `landmarks is not None` branch above - so the bearing
            # and the operator association describe the same instant as the
            # label, not a stale one.
            palm_x, _palm_y = palm_center(landmarks)
            dispatch(make_intent(
                display_command,
                display_conf,
                timestamp=now,
                armed=armed,
...
# DOWNSTREAM CHANGE: no `if __name__ == "__main__"` entry point. This is a
# package module now, not a script - `python3 gesture_control.py` would fail on
# the relative imports anyway. The desk driver is scripts/run_gesture.py, which
# is to this producer what replay_bag.py is to the perception one: a way to run
# the whole chain with no robot and no ROS.