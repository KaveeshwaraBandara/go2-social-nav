...
This module deliberately imports nothing FROM THIS PROJECT. It sits at the
bottom of the dependency graph so that gesture_common, gesture_model,
collect_gestures, train_gestures and gesture_control can all import from it
without any risk of a circular import. (`os` is stdlib and carries no such
risk; it is here only to resolve model paths from the environment, which the
packaged build needs — see MODEL_PATH.)
...
# DOWNSTREAM CHANGE (go2-social-nav). Upstream this is the bare relative path
# "gesture_model.pkl", which resolves against the CWD - fine when you always run
# `python3 gesture_control.py` from the repo root, broken the moment the package
# is installed and launched from anywhere.
#
# The default points at the BIND-MOUNTED SOURCE TREE, matching how the
# perception producer resolves its rig calibration (scripts/replay_bag.py,
# DEFAULT_CALIB). ros2_ws is mounted live into the container, so committing the
# bundle to gesture_lab/model/ makes it work with no rebuild and no env var.
#
# Unlike the .task bundles this is NOT baked into the image: it is the trained
# artifact of the research, code-adjacent and small, so it belongs in git next
# to the code that defines its feature layout — the same argument that keeps
# go2_calib.npz in the repo while the rosbags stay out of it.
MODEL_PATH = os.environ.get(
    "GO2_GESTURE_MODEL",
    "/home/dev/ros2_ws/src/gesture_lab/model/gesture_model.pkl",
)
...
# ##########################################################################
# DOWNSTREAM ADDITIONS (go2-social-nav, Phase 11)
#
# Everything above this line is upstream (Lasan-Perera/go2-gesture-control,
# pinned at a0c9bdc) with only MODEL_PATH changed. Everything below exists
# because the intent contract needs it and the standalone repo did not.
#
# Kept in one block so a re-sync with upstream is a mechanical diff rather
...
# --------------------------------------------------------------------------
# Upstream expects both .task bundles in a models/ directory beside the source,
# downloaded by hand per the README. Here they are BAKED INTO THE IMAGE at
# /opt/models (docker/Dockerfile, Phase-11 layer), the same convention the
# perception producer uses for the YOLO weights: a pinned asset at a fixed path,
# so a run never silently depends on the network or on whatever is newest
# upstream. 17 MB of binary blobs also do not belong in the repo.
#
# The env vars let a laptop checkout point at a local models/ instead.
HAND_LANDMARKER_PATH = os.environ.get(
    "GO2_HAND_LANDMARKER", "/opt/models/hand_landmarker.task"
)
POSE_LANDMARKER_PATH = os.environ.get(
    "GO2_POSE_LANDMARKER", "/opt/models/pose_landmarker_full.task"
)

# --------------------------------------------------------------------------
# Camera geometry — needed for GestureIntent.operator_bearing
# --------------------------------------------------------------------------
# The producer must report WHERE the operator is, not just what they said, so
# the controller can associate the intent with a perception Track. Bearing is
# the only spatial quantity a monocular hand pipeline can emit honestly (see
# gesture_intent/intent.py on why there is no operator_distance), and it needs
# the horizontal field of view to convert an image column into an angle.
#
# 69.0 deg is the RealSense D435i COLOUR stream (the robot's camera). A laptop
# webcam is typically 55-65, so bearings measured at the desk are slightly
# compressed - fine for development, wrong for a logged experiment. Override
# rather than guess.
CAMERA_HFOV_DEG = float(os.environ.get("GO2_CAMERA_HFOV_DEG", "69.0"))

# gesture_control.py mirrors every frame (cv2.flip) so the operator sees a
# natural "selfie" view of themselves. That flip is a DISPLAY convenience, and
# it inverts the sign of every horizontal measurement taken from the image.
#
# Left unhandled this is the classic silent sign error: the robot turns toward
# an operator standing on its other side, and nothing in the pipeline complains.
# intent_adapter.bearing_from_image_x() un-mirrors explicitly, gated on this
# flag, so the convention is stated once and in one place.
